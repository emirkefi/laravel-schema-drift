...
$liveTypeMismatch['users']['columns']['is_active']['type'] = 'integer';
$diffs = $diffEngine->compare($liveTypeMismatch, $expected);
test('Detects TYPE_MISMATCH correctly', count($diffs) === 1 && $diffs[0]->issueType === 'TYPE_MISMATCH' && $diffs[0]->expected === 'boolean' && $diffs[0]->actual === 'integer', $passed, $failed);
